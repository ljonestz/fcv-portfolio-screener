...
<div class="top-bar">
  ...
  <span class="top-bar-badge">DJIBOUTI · FRAGILE CONTEXT</span>
</div>
...
<div class="page-wrap">
  ...
  <div class="report-section" id="overview">
    ...
    <p>This report presents a systematic FCV (Fragility, Conflict and Violence) Portfolio Assessment of the World Bank Group's Djibouti portfolio — {stats['n']} operations approved between 2015 and 2024, all under Investment Project Financing (IPF) instruments. Djibouti occupies a strategically critical position in the Horn of Africa: it hosts military bases for the United States, France, and China, serves as the primary maritime gateway for landlocked Ethiopia, and shares borders with Ethiopia, Eritrea, Somalia, and Yemen. This combination of strategic rents, acute youth unemployment, governance fragility, and exposure to regional conflict spillovers places Djibouti firmly within the FCAS (Fragile and Conflict-Affected States) classification — a context that raises the bar for FCV integration in Bank operations.</p>

    <p>Each project was assessed against the WBG FCV Sensitivity and Responsiveness Screener, which evaluates eight analytical dimensions across two composites. <strong>FCV Sensitivity</strong> (Dimensions 1–3) measures how well a project understands Djibouti's fragility dynamics — its conflict drivers, political economy, and do-no-harm risks. <strong>FCV Responsiveness</strong> (Dimensions 4–8) measures whether the operational design actually adapts to that context: the theory of change, targeting logic, implementation flexibility, results framework, and One WBG integration. Scores run from 1 to 10; 7.0 is the high-performance threshold.</p>

    <p>The headline result is a portfolio that <strong>partially understands Djibouti's FCV context but inconsistently adapts to it</strong>. The average sensitivity score of <strong>{stats['avg_s']:.1f}/10</strong> ({s_rating}) indicates that FCV analysis is engaged — but rarely deeply. The average responsiveness score of <strong>{stats['avg_r']:.1f}/10</strong> ({r_rating}) indicates that this analysis is translating into operational design only sporadically. The {stats['avg_s'] - stats['avg_r']:.1f}-point gap between sensitivity and responsiveness is the central finding of this assessment. On trajectory, both scores show {'a positive trend' if trend_s > 0 else 'a mixed trend'} ({trend_s:+.2f}/yr sensitivity, {trend_r:+.2f}/yr responsiveness), suggesting the WBG FCV Strategy is beginning to have an effect — but the baseline is low enough that further deliberate effort is needed.</p>

    <p>The chart below maps all {stats['n']} projects onto the two-by-two gap matrix. Each dot is a project; the star marks the portfolio average. The distribution shows a near-even split between High FCV Integration and Low FCV Integration clusters — with a small Implementation Gap group — reflecting a portfolio that is polarised rather than uniformly weak. A subset of operations engages Djibouti's FCV context seriously; many others treat it as a generic development context. {n_high} projects achieve High FCV Integration; {n_low} fall into Low FCV Integration; {n_impl_gap} sit in the Implementation Gap quadrant.</p>
    ...
    <p>The polarised distribution is itself a finding: it suggests that FCV quality in Djibouti operations is driven more by individual TTL orientation and sectoral norms than by portfolio-wide institutional standards. Where one project thoroughly analyses Djibouti's fragility drivers and designs adaptive delivery accordingly, the project next door may contain no FCV analysis at all. Closing this variance — not just raising the average — should be the primary objective of quality assurance reforms.</p>
  ...
  <div class="report-section" id="portfolio">
    ...
    <p>The Djibouti portfolio screened here spans a decade of World Bank Group engagement, from 2015 to 2024. All {stats['n']} operations are Investment Project Financing (IPF) — the Bank found no DPF or Programme-for-Results instruments in the Djibouti portfolio for this period. The portfolio covers a wide range of sectors: urban infrastructure and transport, social protection, health, education, water and sanitation, private sector development, and governance and public financial management. This sectoral breadth reflects Djibouti's ambitious development agenda under the 'Vision Djibouti 2035' strategy, which aims to transform the country into a regional logistics and services hub.</p>

    <p>Approval activity peaked in {peak_year} with {peak_year_count} operations, reflecting a period of intensified WBG engagement. Several projects in the portfolio are Additional Financing operations extending successful earlier interventions — particularly in health, social safety nets, and urban development — signalling a degree of continuity in sectoral engagement. One project (P165558, Women and Youth Entrepreneurship) was excluded from screening as no public PDF was available via the WB Documents API; the remaining {stats['n']} projects provide a comprehensive coverage of the active portfolio.</p>
  ...
  <div class="report-section" id="scores">
    ...
    <p>The near-equal split between High FCV Integration ({n_high} projects, {n_high/stats['n']*100:.0f}%) and Low FCV Integration ({n_low} projects, {n_low/stats['n']*100:.0f}%) is a more distinctive pattern than is typically seen in portfolio-level analyses, which tend to be dominated by a single quadrant. It suggests that Djibouti operations come from two distinct populations: those where TTLs have engaged substantively with FCV context and built it into design, and those where FCV analysis is perfunctory or absent. The two trend charts below show whether this distribution is shifting over time. Both sensitivity and responsiveness trends ({trend_s:+.2f}/yr and {trend_r:+.2f}/yr respectively) suggest {'modest improvement in the later cohort' if trend_s > 0 else 'no clear improvement trend yet'}, but year-to-year volatility is high given the small portfolio size.</p>
  ...
  <div class="report-section" id="dimensions">
    ...
    <p>At the dimension level, the portfolio's strongest area is <strong>D{best_dim_id}: {best_dim_name}</strong> (avg {best_dim_score:.1f}/10) — the dimension where FCV thinking is most consistently applied across operations. The weakest is <strong>D{worst_dim_id}: {worst_dim_name}</strong> (avg {worst_dim_score:.1f}/10), which is where the most systematic design gap lies and where targeted quality review would have the highest leverage. The pattern across all eight dimensions follows the expected sensitivity-responsiveness gradient: sensitivity-side dimensions (D1–D3) generally score higher than responsiveness-side dimensions (D4–D8), even though the gap is narrower here than in more acute FCV settings.</p>

    <p>The radar chart below visualises all eight dimensions simultaneously, comparing the early cohort (2015–2019) against the later cohort (2020–2024) and the portfolio average. This framing — rather than the instrument comparison used for portfolios with multiple instrument types — reveals whether the WBG FCV Strategy's post-2020 guidance translated into better operational design in Djibouti. Look for a wider arc in the later cohort on responsiveness dimensions (D4–D8) as the clearest indicator of strategy impact.</p>
    ...
    <p>The table and heatmap below provide greater granularity. In the heatmap, each column is a project and each row is a dimension. Horizontal patterns — rows that are consistently orange or red — signal portfolio-wide weaknesses. Vertical patterns — columns that are uniformly green or red — identify strong and weak outlier projects worth examining individually. The most diagnostically useful row to examine is D8 (One WBG Integration): given Djibouti's relatively more developed private sector compared to active conflict states, IFC engagement in energy, logistics, and financial services should be a stronger feature of project design than the scores suggest.</p>
  ...
  <div class="report-section" id="redflags">
    ...
    <p>Across the {stats['n']}-project portfolio, {stats['any_rf']} operations trigger at least one red flag. The most prevalent is <strong>{most_common_rf}: {most_common_rf_label}</strong>, which appears in {most_common_rf_count} of {stats['n']} projects ({most_common_rf_pct:.0f}%). The chart below shows the frequency of each flag across the portfolio. In Djibouti's specific context, the most concerning pattern is not which flags are triggered most often, but that a quarter of operations carry red flags at all — in a country where Djibouti's high inequality, centralised political economy, and ethnic diversity create genuine pathways to harm if project design does not explicitly account for them. TTL-level guidance on the most common flag types — particularly distributional analysis and results framework design — would be the most efficient intervention point.</p>
  ...
  <div class="report-section" id="cohorts">
    ...
    <p>Since the entire Djibouti portfolio is IPF, the most analytically useful cut of the data — beyond sector-level breakdowns — is temporal: has the quality of FCV integration improved between the early cohort (2015–2019, n={stats['n_early']}) and the later cohort (2020–2024, n={stats['n_late']})? The 2020 WBG FCV Strategy and its associated operational guidance raised analytical expectations for operations in fragile contexts, making the post-2020 period the natural treatment group for assessing strategy impact.</p>

    <p>The results show {'improvement in both composites' if stats['late_avg_s'] > stats['early_avg_s'] and stats['late_avg_r'] > stats['early_avg_r'] else 'a mixed picture'} across the two cohorts. The early cohort averages {stats['early_avg_s']:.1f}/10 on sensitivity and {stats['early_avg_r']:.1f}/10 on responsiveness. The later cohort averages {stats['late_avg_s']:.1f}/10 on sensitivity and {stats['late_avg_r']:.1f}/10 on responsiveness. {'The increase in both composites is consistent with strategy impact, though portfolio size limits statistical confidence.' if stats['late_avg_s'] > stats['early_avg_s'] else 'The lack of clear improvement suggests that the FCV Strategy has not yet systematically raised design quality in Djibouti operations.'} The box plots below show the full score distributions for each cohort, including spread and outliers — a more complete picture than averages alone.</p>
  ...
  <div class="report-section" id="conclusions">
    ...
    <p>The Djibouti portfolio demonstrates {'moderate' if 4 <= stats['avg_s'] < 7 else 'strong' if stats['avg_s'] >= 7 else 'weak'} FCV sensitivity and {'moderate' if 4 <= stats['avg_r'] < 7 else 'strong' if stats['avg_r'] >= 7 else 'weak'} FCV responsiveness across {stats['n']} screened operations. The portfolio is not failing on FCV — but it is inconsistent in ways that suggest institutional factors (TTL orientation, sectoral norms, quality review standards) rather than context understanding drive FCV quality. Given Djibouti's FCAS classification and the specificity of its fragility pathways, this inconsistency is a risk to portfolio effectiveness.</p>

    <p>Five priorities stand out from this analysis:</p>

    <ol>
      <li><strong>Raise the floor, not just the ceiling.</strong> The most pressing issue in the Djibouti portfolio is variance — the gap between the best-performing and worst-performing operations is very wide. Portfolio management should focus on systematically raising the lower tail: operations that score below 4.0 on either composite in a recognised FCAS context should be flagged for mandatory FCV Country Coordinator review before concept note approval.</li>
      <li><strong>Address Djibouti's specific fragility pathways explicitly.</strong> Youth unemployment, ethnic marginalisation (Afar communities in particular), climate-driven displacement, and the political economy of strategic rent distribution are Djibouti's defining fragility drivers. These should be named — not implied — in every PAD's risk and contextual analysis section. Generic 'fragility' language that applies to any FCAS context is insufficient.</li>
      <li><strong>Strengthen do-no-harm and distributional analysis.</strong> The most prevalent red flags relate to missing distributional analysis and elite capture risks. Djibouti's concentrated political economy and the dominance of a single ruling family in resource allocation make elite capture a genuine design risk in social protection, infrastructure, and private sector operations. Explicit mitigation plans should be required, not optional.</li>
      <li><strong>Build adaptive management into results frameworks.</strong> Dimension 7 (Results Framework and Adaptive Management) is consistently among the weakest dimensions. Given Djibouti's volatility — driven by regional conflict spillovers and climate shocks — results frameworks that cannot be adjusted to FCV-relevant shocks are not fit for purpose. Contingency clauses, FCV-adjusted indicators, and mid-term review triggers should be standard provisions.</li>
      <li><strong>Exploit One WBG opportunities more systematically.</strong> Djibouti's positioning as a logistics hub, combined with IFC's presence in the energy and financial sectors, creates genuine opportunities for joint WBG programming that are not being captured in most IPF PADs. The CMU should develop a standing coordination mechanism with IFC to ensure that investment-side analysis in PADs reflects IFC's pipeline and risk assessments.</li>
    </ol>
  </div>
