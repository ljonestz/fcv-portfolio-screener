...
<div class="page-wrap">
  ...
  <div class="report-section" id="redflags">
    ...
    <p>Beyond composite scores, the screener flags specific design failures that carry programmatic risk in fragile settings. These are not minor omissions — they represent concrete gaps that could cause harm, undermine effectiveness, or erode community trust. A project can score adequately on average and still carry a critical red flag: for example, naming a harm pathway but providing no mitigation, or deploying a results framework with no FCV-adjusted indicators.</p>

    <p><strong>{stats['any_rf']} of {stats['n']} operations ({stats['any_rf']/stats['n']*100:.0f}%) trigger at least one red flag</strong> — a majority of the portfolio, not a marginal subset. The most prevalent is <strong>{most_common_rf}: {most_common_rf_label}</strong>, appearing in {most_common_rf_count} of {stats['n']} projects ({most_common_rf_pct:.0f}%). This is a structural weakness, not an outlier pattern: it reflects a portfolio where results frameworks are designed for standard development monitoring rather than for FCV-sensitive adaptive management. In Djibouti's context — where the political economy of strategic rents, ethnic marginalisation, and climate-driven displacement create genuine pathways to harm — the prevalence of red flags is a direct indicator of design risk. Targeted TTL guidance on the most common flag types, particularly results framework design and distributional analysis, would have the highest return on quality investment.</p>
