"""
Djibouti FCV Portfolio — Data Collection Script (Steps 1–3)
Date: 2026-03-16

Steps performed:
  1. Fetch project portfolio from WB Projects API (country code: DJ)
  2. Filter to IPF, DPF, P4R instruments, approval years 2015–2024
  3. Fetch best-available project document (PAD > PD > ICR > PP > CN)
  4. Extract PDF text using PyMuPDF (fitz)

Outputs (written to this script's directory):
  - filtered_djibouti_portfolio.json   — project metadata
...
START_YEAR     = 2015
END_YEAR       = 2024
MAX_CHARS      = 120_000   # ~80–100 pages of PDF text
...
def fetch_portfolio() -> list:
    """Fetch all WB projects for Djibouti and filter to target instruments/years."""
...
def _title_words(title: str) -> set:
    ...
    Keeps 'djibouti' to distinguish country-specific docs but
    strips very generic WB boilerplate words.
    """
    import re
...
def fetch_all_djibouti_pads() -> list:
    """
    The WB Documents API does not reliably link PADs to project IDs.
    Fetch all English Djibouti PADs via text search.
    ...
    for docty in ['Project+Appraisal+Document', 'Program+Document',
        ...
        url = (
            ...
            f'?format=json&qterm=Djibouti&docty_exact={docty}'
            f'&rows=100&fl=id,docty,display_title,pdfurl,docdt,lang'
        )
        try:
            data = fetch_json(url)
    ...
    print(f'    Pre-fetched {len(unique)} unique Djibouti PADs')
    return unique
...
def get_best_direct_doc(pid: str) -> dict | None:
    ...
    Validates returned docs are Djibouti-specific (title must contain 'djibouti' or 'djib').
    """
    url = (
        f'https://search.worldbank.org/api/v2/wds'
    ...
    for d in docs:
        ...
        if d.get('lang', 'English') != 'English':
            continue
        # Validate the document is actually about Djibouti
        title_lower = (d.get('display_title', '') or '').lower()
        if 'djibouti' not in title_lower and 'djib' not in title_lower:
            continue
...
def get_keyword_doc(project_name: str) -> dict | None:
    """
    Last-resort fallback: search WB Documents by project name keywords + country=DJ.
    Accepts Project Paper, ICR, or ISR if title is Djibouti-specific.
    """
    ...
    url = (
        f'https://search.worldbank.org/api/v2/wds'
        f'?format=json&qterm={qterm}&countrycode_exact=DJ&rows=10'
...
def fetch_documents(portfolio: list) -> tuple[list, list]:
    ...
    For each project, find the best document:
      1. PAD via title-matching against pre-fetched Djibouti PADs (strict ≥60% recall)
      2. ISR or Project Paper via direct project_id lookup
    Returns (targets, excluded).
    """
    print(f'\nStep 2: Fetching document URLs for {len(portfolio)} projects...')

    print('  Pre-fetching all Djibouti PADs from WB Documents API...')
    all_pads = fetch_all_djibouti_pads()
    ...
    for i, p in enumerate(portfolio, 1):
        ...
        print(f'  [{i:02d}/{len(portfolio)}] {pid}: {name[:50]}')
        ...
        # Method 2: direct project_id lookup (ISR / Project Paper), Djibouti-validated
        if doc is None:
            doc = get_best_direct_doc(pid)
            if doc:
                method = 'direct'

        # Method 3: keyword search with country=DJ filter
        if doc is None:
...
def extract_pdf_text(url: str, max_chars: int = MAX_CHARS) -> str:
...
def extract_all_texts(targets: list) -> dict:
    ...
    for i, t in enumerate(targets, 1):
        ...
        if out_file.exists():
            ...
            results[pid] = text
            print(f'  [{i:02d}/{len(targets)}] {pid}: cached ({len(text):,} chars)')
            continue

        print(f'  [{i:02d}/{len(targets)}] {pid}: {t["doc_type"][:40]}...', end='', flush=True)
        try:
...
def main():
    print('=' * 60)
    print(f'Djibouti FCV Portfolio — Data Collection')
