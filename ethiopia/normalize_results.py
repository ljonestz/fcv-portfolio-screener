"""
Ethiopia FCV Portfolio — Normalize and Merge Screening Results
Date: 2026-03-16

Globs individual per-project result files (screening_results_P*.json)
produced by the 1-per-agent screening approach, and merges them into
one canonical normalized JSON file.

Also recalculates gap_matrix_cell from scores to correct any agent errors.

Output: 20260316_ethiopia_screening_results_normalized.json
"""
...
OUTPUT_FILE = SCRIPT_DIR / '20260316_ethiopia_screening_results_normalized.json'
...
def main():
    print('Ethiopia FCV Portfolio — Normalizing Screening Results')
    print('=' * 55)

    # Glob individual per-project result files (1-per-agent approach)
    result_files = sorted(SCRIPT_DIR.glob('screening_results_P*.json'))
    print(f'Found {len(result_files)} result files')

    if not result_files:
        print('  ERROR: No screening_results_P*.json files found.')
        print('  Run FCV screening agents first (see CLAUDE.md Step 4).')
        return
    ...
    for f in result_files:
        try:
            ...
            if isinstance(data, list):
                raw.extend(data)
                print(f'  {f.name}: {len(data)} records (list)')
            else:
                raw.append(data)
                print(f'  {f.name}: 1 record (object)')
    ...
    instr_counts = Counter(r['instrument_category'] for r in normalized)
    print('\nInstrument breakdown:')
    for instr, n in instr_counts.most_common():
        print(f'  {n:2d}x {instr}')
