...
<div class="page-wrap">
  ...
  <div class="report-section" id="redflags">
    ...
    <p>Somalia's red flag profile is notably restrained: only 28% of the portfolio triggers any flag at all — a lower rate than Ethiopia (80%) or Djibouti (68%). This likely reflects the maturity of the Somalia programme: task teams operating in one of the world's most prominent FCS countries have been acutely sensitised to conflict and fragility risks over two decades of engagement, and the FCV Strategy's operational guidance is well embedded in Somalia operations.</p>

    <p><strong>RF2 — Missing Distributional Analysis (18%)</strong> and <strong>RF4 — Elite Capture Unmitigated (15%)</strong> are the two most common flags, albeit at moderate rates. RF2 reflects the challenge of ex-ante distributional analysis in a context where reliable household survey data is scarce and where IDP, pastoral, and clan-based population distributions shift rapidly. The seven projects triggering RF2 tend to be those with broad geographic targeting where the distributional consequences of resource allocation across clan territories are not explicitly modelled. RF4 captures a related but distinct risk: Somalia's clan-based governance and procurement systems mean that elite capture is not hypothetical but endemic. The six projects triggering RF4 acknowledge this dynamic but do not propose specific countermeasures beyond standard fiduciary controls — which, in Somalia's context, are insufficient given the parallel clan-based accountability structures that coexist with formal government systems.</p>

    <p><strong>RF1 — Unmitigated Conflict Risk (0%).</strong> No project triggers this flag. The near-zero rate is, in Somalia's case, more credible than in many FCV portfolios. The portfolio-average D2 score (Do No Harm and Conflict Risk) of 6.2/10 — approaching the 'Substantially Addressed' threshold — indicates that most projects engage meaningfully with conflict risk analysis and propose specific mitigation measures. As with RF1 generally, a 0% rate should be read with nuance: it reflects an absence of documented unmitigated conflict pathways, not an absence of risk.</p>

    <p><strong>RF3 — OP 7.30 Weakly Handled (0%).</strong> RF3 is a narrow, evidence-based flag that triggers only when a project document explicitly names engagement with — or transfer of resources to — a <em>de facto</em> or unconstitutionally constituted authority, the scenario governed by World Bank Operational Policy 7.30. It is not a general safeguards or exclusion check. An earlier screening had flagged the SHIIP infrastructure corridor project (P173119) under RF3 on the basis of its geographic footprint across southern and central Somalia — areas where al-Shabaab exerts administrative or informal governance functions. However, following re-screening with the strict OP 7.30 definition, the project document does not explicitly name engagement with al-Shabaab or any other non-state authority as part of project implementation or resource flows. The flag was accordingly corrected to false. The genuine operational complexity of working in al-Shabaab-influenced areas is captured in P173119's D2 dimension score rather than in this binary flag. That no project in the Somalia portfolio triggers RF3 under the strict definition is consistent with the World Bank's standard practice of not documenting engagement with non-state armed groups in formal project documents, even where such dynamics shape the operational environment.</p>

    <p><strong>RF5 — Macro Framework Unrealistic (2%).</strong> A single project triggers this flag. In a portfolio dominated by IPF operations — where results indicators are tied to project-specific outputs rather than macro-level policy conditions — a near-zero RF5 rate is structurally expected. DPF operations, which would face the greatest macro-framework risk, represent a small share of the Somalia portfolio. This flag is more diagnostically useful in portfolios with a significant P4R or DPF component, as seen in Ethiopia.</p>
  </div>
