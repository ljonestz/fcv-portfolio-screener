...
<div class="page-wrap">
  ...
  <div class="report-section" id="redflags">
    ...
    <p>Somalia's red flag profile is notably restrained: only 28% of the portfolio triggers any flag at all — a lower rate than Ethiopia (86%) or Djibouti (73%). This likely reflects the maturity of the Somalia programme: task teams operating in one of the world's most prominent FCS countries have been acutely sensitised to conflict and fragility risks over two decades of engagement, and the FCV Strategy's operational guidance is well embedded in Somalia operations.</p>

    <p><strong>RF2 — Missing Distributional Analysis (15%)</strong> and <strong>RF4 — Elite Capture Unmitigated (12%)</strong> are the two most common flags, albeit at moderate rates. RF2 reflects the challenge of ex-ante distributional analysis in a context where reliable household survey data is scarce and where IDP, pastoral, and clan-based population distributions shift rapidly. The six projects triggering RF2 tend to be those with broad geographic targeting where the distributional consequences of resource allocation across clan territories are not explicitly modelled. RF4 captures a related but distinct risk: Somalia's clan-based governance and procurement systems mean that elite capture is not hypothetical but endemic. The five projects triggering RF4 acknowledge this dynamic but do not propose specific countermeasures beyond standard fiduciary controls — which, in Somalia's context, are insufficient given the parallel clan-based accountability structures that coexist with formal government systems.</p>

    <p><strong>RF1 — Unmitigated Conflict Risk (2%)</strong> affects a single project. The near-zero rate is, in Somalia's case, more credible than in other FCV portfolios. The portfolio-average D2 score (Do No Harm and Conflict Risk) of 6.2/10 — approaching the 'Substantially Addressed' threshold — indicates that most projects engage meaningfully with conflict risk analysis and propose specific mitigation measures. The one RF1 trigger therefore appears to be a genuine outlier rather than evidence of systematic under-reporting.</p>

    <p><strong>RF3 — OP 7.30 Weakly Handled (2%).</strong> RF3 is a narrow, evidence-based flag that checks specifically whether a project engages with or transfers resources to a <em>de facto</em> or unconstitutionally constituted authority — the situation governed by World Bank Operational Policy 7.30. It is not a general safeguards or exclusion check; it triggers only when the project document itself references such an engagement and fails to adequately address the resulting governance and legitimacy risks. In Somalia's case, the single RF3 trigger — most plausibly the SHIIP infrastructure corridor project (P173119) — is credible. Operating in conflict-contested areas where al-Shabaab exerts administrative or informal governance functions makes OP 7.30 engagement directly in scope; the project's geographic footprint across southern and central Somalia places it in areas where the line between state and non-state authority is contested. That only one project triggers RF3 in a portfolio of this size is, if anything, reassuring given the prevalence of non-state authority across Somalia's project areas. This is also the portfolio where the narrow OP 7.30 reading is most applicable — unlike Ethiopia and Djibouti, where screening agents drifted from the strict definition and applied RF3 to general exclusion and safeguards gaps unrelated to de facto authority engagement.</p>

    <p><strong>RF5 — Macro Framework Unrealistic (0%).</strong> No project triggers this flag. In a portfolio dominated by IPF operations — where results indicators are tied to project-specific outputs rather than macro-level policy conditions — the absence of RF5 is structurally expected. DPF operations, which would face the greatest macro-framework risk, represent a small share of the Somalia portfolio. This flag is more diagnostically useful in portfolios with a significant P4R or DPF component, as seen in Ethiopia.</p>
  </div>
