"""
Somalia RF3 Recheck -- Patch Script
Date: 2026-03-18

Reads 1 recheck JSON file (P173119) and patches the normalized results:
...
Run from: somalia/ folder or anywhere (uses Path(__file__).parent)
"""
...
NORMALIZED_FILE = SCRIPT_DIR / '20260314_somalia_screening_results_normalized.json'
BACKUP_FILE     = SCRIPT_DIR / f'20260314_somalia_screening_results_normalized.BACKUP_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'

RECHECK_PIDS = ['P173119']
...
def extract_scores(recheck: dict):
    if 'composites' in recheck:
        ...
        sens = s.get('sensitivity', {}).get('numeric_score') or s.get('Sensitivity', {}).get('numeric_score')
        resp = s.get('responsiveness', {}).get('numeric_score') or s.get('Responsiveness', {}).get('numeric_score')
...
def main():
    print('Somalia RF3 Recheck -- Patching Normalized Results')
    print('=' * 52)
    ...
    for pid in RECHECK_PIDS:
        ...
        if new_sens is not None:
            print(f'    Sensitivity:    {orig.get("sensitivity_score")} -> {new_sens}')
            print(f'    Responsiveness: {orig.get("responsiveness_score")} -> {new_resp}')
    ...
    print(f'\nPost-patch RF3: {rf3_count}/{len(normalized)}')
    print(f'Post-patch any_rf: {any_rf_count}/{len(normalized)}')
